 ...
 *
 * Edited by Squirrel to add the option possition: 'top' on Line 244
 */
(function ($) {
    ...
    $.fn.mentionable = function (strUsersUrl, oOpts, fnOnComplete) {
        ...
        if (isAndroid) {
            this.bind('input', function (e) {
                const char = this.value.charCodeAt(this.value.length - 1);
                e.keyCode = char;
                e.charCode = char;
    ...
    function showUserFrame() {
        ...
        } else if (oOptions.position == 'top') { // edited by squirrel, since this option was not available but is needed for chat.
            eUserList.css('left', 0);
            eUserList.css('bottom', eTextArea.outerHeight(true));
            eUserList.css('width', eTextArea.outerWidth());
        }
    ...
    function populateItems(strKeyword) {
        if (strKeyword.length > oOptions.minimumChar && (!oOptions.maxTags || $('[name="mentioned_id[]"]').length < oOptions.maxTags)) {
            ...
            if (fnOnComplete == undefined)
                fnOnComplete = function (oData) { fillItems(Array.isArray(oData) ? oData : (oData.items || [])); }

            if (oOptions.token !== 'undefined')
                oData.token = oOptions.token;
            //added optional postId param when using mentions in a reply
            let parentPostId = eUserListWrapper.closest('form').find('[name=parentId]');

            if (parentPostId.length) {
                oData.postId = parentPostId.val();
            }

            if (mainData.channel !== undefined) {
                oData.channelId = mainData.channel.id;
            }
        ...
        if (oData.length > 0) {
            ...
            $.each(oData, function (iKey, oValue) {
                eUserList.append('<li data-friend-id="' + oValue.id
                    + '"><img src="' + oValue.img + '"><span>'
                    + oValue.label + '</span></li>');
            });
            eUserList.find('li:first-child').attr('class', 'active');
            bindItemClicked();
        } else {
            eUserList.append("<li class='not-found'>No user found</li>");
        }
    }
    ...
    function selectUser(eUserItem) {
        var friendId = $(eUserItem).data('friend-id');
        var friendsIds = $(eTextArea).data('friends-ids');
        ...
        // Remove any duplicate @. Fixes issue on Android
        strReplacedText = strReplacedText.replace(/(@)\1+/g, '@');

        eTextArea.focus();
        eTextArea.val(strReplacedText.trim() + ' ');
        friendsIds.push(friendId);
        $(eTextArea).data('friends-ids', friendsIds);
